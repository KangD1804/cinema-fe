...
import { logoutAction } from "../../redux/actions/UserManagementActions";
...
export default function Header(props) {
  ...
  const account = useSelector(
    (state) => state.UserManagementReducer.account
  );
  ...
  const renderMenuControl = () => {
    ...
    if (userLoginData && JSON.parse(userLoginData).role === "QuanTri") {
      return (
      ...
      return (
        <Fragment>
          ...
          >
            ...
            <span className="login__text">{account}</span>
          </div>
          <Popper
            open={open}
